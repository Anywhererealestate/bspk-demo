...
`;function StylesProviderEra(){const styleEmentRef=reactExports.useRef(null);return reactExports.useEffect(()=>(styleEmentRef.current=document.createElement("style"),styleEmentRef.current.setAttribute("data-bspk","era"),styleEmentRef.current.innerHTML=brandStyle+baseStyle,document.head.appendChild(styleEmentRef.current),document.body.style.display="",()=>{styleEmentRef.current&&document.head.removeChild(styleEmentRef.current)}),[]),jsx(Fragment,{})}StylesProviderEra.bspkName="StylesProviderEra";export{StylesProviderEra};
